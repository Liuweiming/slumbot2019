...
static void Write(int st, const string &bucketing, uint32_t *assigments,
                  int *indices, int num_buckets) {
...
static void Usage(const char *prog_name) {
  fprintf(
      stderr,
      "USAGE: %s <game params> <street> <num clusters> <bucketing> <features> "
...
int main(int argc, char *argv[]) {
  ...
  char buf[500];
  sprintf(buf, "%s/features.%s.%i.%s.%i", Files::StaticBase(),
  ...
  Write(st, bucketing, assignments, indices, num_clusters);
